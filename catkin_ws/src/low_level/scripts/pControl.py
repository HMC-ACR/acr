...
class PointTracker:
    ...
    def odom_callback(self, odom):
        if (self.current_target.position.x == 0 and self.current_target.position.y == 0):
            ...
            print('Initialize target position to non-origin position')
        else:
            # We have a target, and we have moved, so recalc errors
            ...
            # TODO(apham): Verify that angle wrapping is working properly
            if (self.alpha >= pi):
                self.alpha -= pi
            elif (self.alpha <= -pi):
                self.alpha += pi
    ...
    def cmd_vel_pub(self):
        ...
        # TODO(apham): We may want to consider beta (the orientation of the vehicle error) in the
        # future
        ...
        # TODO(apham): We probably don't need to have this hack if we tune the gains properly
